...
def ouput_fusion_transcript(gpd_list, fusion_count, output_gpd_fl):
    if fusion_count == "all":
        for gpd in gpd_list:
            print(gpd, file=output_gpd_fl)
    else:
        if int(fusion_count) >= len(gpd_list):
            sys.stdout.write("The total count of genereated fusion transcripts is " + str(len(
                gpd_list)) + ". Even though you set a big number, we only output all fusion transcript for you!!!\n")
            for gpd in gpd_list:
                print(gpd, file=output_gpd_fl)
        else:
...
def do_inputs():
    ...
    parser = argparse.ArgumentParser(
        description="Generate fusion transcripts using gpd file. One isoform with most exon count per gene is used, and total number of generated fusion transcripts is equal to (total gene count)*(total gene count - 1)",
