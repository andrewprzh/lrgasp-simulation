...
def convert_gpd_to_fasta(input_fa_fl, input_gpd_fl, output_fa_fl):
    ...
    dic_nogatc_code = {"U": ["T"], "I": ["G"], "R": ["A", "G"], "Y": ["C", "T"], "S": ["G", "C"], "W": ["A", "T"],
                       "K": ["G", "T"], "M": ["A", "C"], "B": ["C", "G", "T"], "D": ["A", "G", "T"],
                       "H": ["A", "C", "T"], "V": ["A", "C", "G"], "N": ["A", "C", "G", "T"], "X": ["A", "C", "G", "T"]}
    for line in input_gpd_fl:
        gene, iso, chr, strand, tss, tts, cds_start, cds_end, exon_number, exon_start, exon_end = line.strip().split(
            "\t")[:11]
        for h in ["Allele1", "Allele2"]:
            ...
            print(">" + new_iso, file=output_fa_fl)
            for j in range(0, len(seq), 80):
                print(seq[j:j + 80], file=output_fa_fl)
...
def do_inputs():
    parser = argparse.ArgumentParser(description="Generate fasta sequence for each allele of isoforms.",
                                     formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-a', '--input_fasta', type=argparse.FileType('r'), required=True,
                        help="Input: genome fasta file. Chromosome Name format = chromosomeID + 'tab-split' + alleleID")
    parser.add_argument('-g', '--input_gpd', type=argparse.FileType('r'), required=True,
                        help="Input: annotation gpd file")
    parser.add_argument('-o', '--output_fasta', type=argparse.FileType('w'), required=True,
                        help="Output: isoform fasta file. Isoform is renamed as isoform ID + 'space-tab' + alleleID")
