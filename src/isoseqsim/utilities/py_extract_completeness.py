...
def extract_end_completeness(input_anno_fl, input_anno_c_fl, input_lr_fl, output_5cpt_fl, output_3cpt_fl):
	...
	for line in input_anno_c_fl:
		gene, iso, chr, strand, tss, tts, lr_set, lr_count, exon_count, exon_start_set, exon_end_set = line.strip().split(
			"\t")[:11]
	...
	for cpt5_uniq in cpt5_uniq_list:
		...
		if cpt5_uniq <= 100 and freq >= 0.0001:
			print(str(cpt5_uniq) + "\t" + str(freq), file=output_5cpt_fl)
	for cpt3_uniq in cpt3_uniq_list:
		...
		if cpt3_uniq <= 100 and freq >= 0.0001:
			print(str(cpt3_uniq) + "\t" + str(freq), file=output_3cpt_fl)
...
def do_inputs():
	parser = argparse.ArgumentParser(
		description="Extract the completeness information at 5' and 3'end of transcripts. Output deleted nucleotide number and its frequency. Note: only output when the deleted nucleotide number is <= 100 and the frequency is >= 0.0001",
		formatter_class=argparse.ArgumentDefaultsHelpFormatter)
	parser.add_argument('-a', '--input_anno', type=argparse.FileType('r'), required=True,
						help="Input: annotation file, gpd format")
	parser.add_argument('-c', '--input_anno_c', type=argparse.FileType('r'), required=True,
						help="Input: annotation file with assigned long read ID(column 7th), gpd format")
	parser.add_argument('-l', '--input_lr', type=argparse.FileType('r'), required=True,
						help="Input: aligned long read file, gpd format")
	parser.add_argument('-5', '--output_cpt5', type=argparse.FileType('w'), required=True,
						help="Output: 5'end completeness stat file")
	parser.add_argument('-3', '--output_cpt3', type=argparse.FileType('w'), required=True,
						help="Output: 3'end completeness stat file")
