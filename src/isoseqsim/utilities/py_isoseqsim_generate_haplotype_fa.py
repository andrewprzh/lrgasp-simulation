...
def generate_haplotype_fasta(input_fa_fl, input_vcf_fl, individual_id, output_fa_fl):
    ...
    for chr in chr_list:
        ...
        if chr in dic_vcf_info.keys():
            ...
            for i in range(0, ploidy):
                print(">" + chr + "\tAllele" + str(i + 1), file=output_fa_fl)
                new_seq = "".join(dic_ploidy[i])
                for j in range(0, len(new_seq), 80):
                    print(new_seq[j:j + 80], file=output_fa_fl)
        else:
            for i in range(0, ploidy):
                print(">" + chr + "\tAllele" + str(i + 1), file=output_fa_fl)
                new_seq = seq
...
def do_inputs():
    parser = argparse.ArgumentParser(
        description="Generate fasta file for each haplotype of multi-ploid samples. Note: VCF file must have the header lines '#CHROM' for showing the individual ID to be used; Only the phased (split by '|' not '/' in GT format of VCF file) single nucleotide substitution site is used.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-g', '--genome', type=argparse.FileType('r'), required=True,
                        help="Input: genome fasta file. Genome sequence")
    parser.add_argument('-v', '--haplotype', type=argparse.FileType('r'), required=True,
                        help="Input: genotype vcf file. Genotype")
    parser.add_argument('-i', '--id', type=str, required=True,
                        help="Individual/Sample ID located in the header line startswith '#CHROM'")
    parser.add_argument('-o', '--output', type=argparse.FileType('w'), required=True,
                        help="Output: multi-ploid fasta file")
